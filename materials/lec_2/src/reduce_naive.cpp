...
int main(int argc, char** argv)
{
    ...
    std::vector<float> h_a(n), h_result(n / local);
    profiler prof;
    ...
    std::ifstream ifs("reduce_naive.cl");
    ...
    reduce = clCreateKernel(program, "reduce_naive", &err);
    checkError(err, "Creating kernel");

    d_a  = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(float) * n, NULL, &err);
    checkError(err, "Creating buffer d_a");

    d_result  = clCreateBuffer(context,  CL_MEM_READ_WRITE,  sizeof(float) * n / local, NULL, &err);
    ...
    prof.tic("naive reduction opencl");
    ...
    err = clEnqueueReadBuffer( commands, d_result, CL_TRUE, 0, sizeof(float) * n / local, h_result.data(), 0, NULL, NULL );  
    checkError(err, "Reading results");

    float sum_gpu = std::accumulate(h_result.begin(), h_result.end(), 0.0);

    prof.toc("naive reduction opencl");
